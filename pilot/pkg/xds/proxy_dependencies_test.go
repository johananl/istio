...
func TestSidecarWaypointPushes(t *testing.T) {
	...
	_, needsCDS := cdsNeedsPush(addressUpdate, sidecar)
	assert.Equal(t, needsCDS, true)
	...
	_, needsCDS = cdsNeedsPush(unrelatedAddressUpdate, sidecar)
	assert.Equal(t, needsCDS, false)
